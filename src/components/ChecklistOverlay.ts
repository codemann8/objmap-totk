...
export default class ChecklistOverlay extends Vue {
  ...
  private lastTierBumpAt: number = Date.now();
  ...
  private readonly tierBumpNoChangeMs: number = 600000;
  ...
  private async refreshOnce(initial: boolean) {
    ...
    try {
      ...
      if (changed) {
        ...
        this.lastTierBumpAt = now;
      } else {
        const noChangeFor = now - this.lastChangeAt;
        const sinceLastBump = now - this.lastTierBumpAt;
        if (noChangeFor >= this.tierBumpNoChangeMs && sinceLastBump >= this.tierBumpNoChangeMs) {
          this.pollIndex = Math.min(this.pollIndex + 1, this.pollBackoffMs.length - 1);
          this.lastTierBumpAt = now;
        }
      }
    } catch (e) {
      // Keep current backoff tier on refresh errors.
    } finally {
      this.scheduleNext();
    }
  }
  ...
  private formatPercentOverall(marked: number, total: number) {
    if (total === 0) {
      return '0%';
    }
    const percent = (100 * marked / total).toFixed(2);
  ...
  private formatPercentList(marked: number, total: number) {
    if (total === 0) {
      return '0%';
    }
    const percent = Math.floor(100 * marked / total);
    return `${percent}%`;
  }
  ...
  get overallPercent() {
    const totals = this.calculateTotals();
    return this.formatPercentOverall(totals.marked, totals.total);
  }
  ...
  listPercent(list: any) {
    const totals = this.listTotals(list);
    return this.formatPercentList(totals.marked, totals.total);
  }
}
